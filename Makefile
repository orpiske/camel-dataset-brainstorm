...
SOURCE_FILE=$(PROJ_PATH)/acquisition/routes.yaml
...
build:
	mvn clean package
	java -jar $(CLI_PATH) package acquisition --artifact $(SOURCE_ARTIFACTS) --ingestion $(SOURCE_FILE) --output-image $(ACQUISITION_OUTPUT_IMAGE) --username $(ORGANIZATION_USER) --password $(REGISTRY_PASSWD)
